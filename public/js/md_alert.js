...
function mdalert(data) {
    ...
    if (data) {
        ...
        if (data.type) {
            content += `<div class="block text-center w-full mt-4">
                            <img class='block my-0 mx-auto' width="120" height="120" src="${base_url}/images/icon/${data.type}.png" alt="imagen">
                        </div>`;
        }
        ...
        if (data.actions) {
            ...
            if (actions.length > 0) {
                actions.forEach((element, index) => {
                    if (element.url) {
                        actions_btns += `<a href="${element.url}" class="px-4 py-2 bg-${element.type}-500 text-white hover:bg-${element.type}-600 rounded-lg border-0 block text-center w-3/4 mx-auto mb-4 shadow-xl uppercase text-white">${element.name}</a>`;
                    } else {
                        actions_btns += `<a href="#" onclick="${element.callback}(${element.params}); return false;" class="px-4 py-2 bg-${element.type}-500 text-white hover:bg-${element.type}-600 rounded-lg border-0 block text-center w-3/4 mx-auto mb-4 shadow-xl uppercase text-white">${element.name}</a>`;
                    }
                });
            }
        }
