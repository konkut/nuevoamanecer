<x-app-layout>
    <x-slot name="title">
        {{ __('word.bankregister.meta.index.title') }}
    </x-slot>
    <x-slot name="metaDescription">
        {{ __('word.bankregister.meta.index.description')}}
    </x-slot>
    <x-slot name="metaKeywords">
        {{ __('word.bankregister.meta.index.keywords')}}
    </x-slot>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('word.bankregister.resource.index') }}
        </h2>
    </x-slot>
    @if (session('success'))
            ...
            <div class="overflow-hidden shadow-xl sm:rounded-lg">
                <div class="container mx-auto p-4">
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="closeDetailsModal('{{$item->bankregister_uuid}}')">
                                                    &times;
                                                </button>
                                                <h1 class="text-lg font-semibold mx-auto">{{ __('word.bankregister.resource.show') }}</h1>
                                            </div>
                                            <div class="py-12 px-4 text-slate-600 rounded-b-2xl shadow-inner md:max-w-2xl p-8">
                                                    <div class="text-center">
                                                        <div>
                                                            <p class="text-sm font-semibold">{{ __('word.bankregister.attribute.name') }}</p>
                                                            <p>{{ $item->name }}</p>
                                                        </div>
                                                        <div class="mt-4">
                                                            <p class="text-sm font-semibold">{{ __('word.bankregister.attribute.total') }}</p>
                                                            <p>{{ $item->total }}</p>
                                                        </div>
                                                        <div class="mt-4">
                                                            <p class="text-sm font-semibold">{{ __('word.bankregister.attribute.owner_name') }}</p>
                                                            <p>{{ $item->owner_name }}</p>
                                                        </div>
                                                        <div class="mt-4">
                                                            <p class="text-sm font-semibold">{{ __('word.bankregister.attribute.account_number') }}</p>
                                                            <p>{{ substr_replace($item->account_number, str_repeat('*', 4), 0, 4) }}
                                                            </p>
                                                        </div>
                                                        <div class="mt-4">
                                                            <p class="text-sm font-semibold ">{{ __('word.bankregister.attribute.status') }}</p>
                                                            <p>{{ $item->status ? '🟢' : '🔴' }}</p>
                                                        </div>
                                                        <div class="mt-4">
                                                        ...
                                                        <div class="mt-4">
                                                            <p class="text-sm font-semibold">{{ __('word.bankregister.attribute.created_at') }}</p>
                                        ...
                                        <div
                                            ...
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="closeModal('{{$item->bankregister_uuid}}')">
                                                    &times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.bankregister.delete_confirmation')}}
                                                    <strong
                                                        id="name-{{$item->bankregister_uuid}}"></strong>{{__('word.general.delete_warning')}}
                                                </p>
                                            </div>
                    ...
                    <div class="pagination-wrapper mt-4">
                        {!! $bankregisters->appends(['perPage' => $perPage])->links() !!}
                    </div>
                </div>
            </div>
        </div>
