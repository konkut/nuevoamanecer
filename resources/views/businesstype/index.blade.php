<x-app-layout>
    <x-slot name="title">
        {{ __('word.businesstype.meta.index.title') }}
    </x-slot>
    <x-slot name="metaDescription">
        {{ __('word.businesstype.meta.index.description')}}
    </x-slot>
    <x-slot name="metaKeywords">
        {{ __('word.businesstype.meta.index.keywords')}}
    </x-slot>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            {{ __('word.businesstype.resource.index') }}
        </h2>
    </x-slot>
    @if (session('success'))
            ...
            <div class="overflow-hidden shadow-xl sm:rounded-lg">
                <div class="container mx-auto p-4">
                    <div class="flex justify-end space-x-2 items-center mb-4">
                        ...
                        @endcan
                        <form method="GET" action="{{ route('businesstypes.index') }}" onchange="this.submit()"
                              class="inline-block">
                        ...
                        <table class="min-w-full border-collapse border-[#2563eb] text-center text-sm">
                            ...
                            <tr class="bg-[#d1d5db]" title="{{__('word.general.title_icon_filter')}}">
                                ...
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.businesstype.attribute.name') }}')">{{ __('word.businesstype.attribute.name') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.businesstype.attribute.user_id') }}')">{{ __('word.businesstype.attribute.user_id') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.businesstype.attribute.created_at') }}')">{{ __('word.businesstype.attribute.created_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.businesstype.attribute.updated_at') }}')">{{ __('word.businesstype.attribute.updated_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.businesstype.attribute.status') }}')">{{ __('word.businesstype.attribute.status') }}</th>
                            ...
                            @foreach($businesstypes as $item)
                                <tr class="hover:bg-[#d1d5db44] transition duration-200">
                                    ...
                                    <td class="border-t border-b border-[#d1d5db] px-2 py-1">
                                        <div class="flex justify-center space-x-1">
                                            @can('businesstypes.index')
                                                <a href="javascript:void(0);"
                                                   title="{{__('word.general.title_icon_show')}}"
                                                   class="bg-green-500 text-white px-2 py-1 rounded text-xs"
                                                   onclick="openDetailsModal('{{$item->businesstype_uuid}}')">
                                            ...
                                            @endcan
                                            @can('businesstypes.edit')
                                                <a href="{{ route('businesstypes.edit',$item->businesstype_uuid) }}"
                                            ...
                                            @endcan
                                            @can('businesstypes.status')
                                                @if($item->status)
                                                    <button type="button"
                                                            class="bg-sky-500 text-white px-2 py-1 rounded text-xs"
                                                            onclick="open_disable_modal('{{ $item->businesstype_uuid }}', '{{ $item->name }}')"
                                                ...
                                                @else
                                                    <button type="button"
                                                            class="bg-sky-500 text-white px-2 py-1 rounded text-xs"
                                                            onclick="open_enable_modal('{{ $item->businesstype_uuid }}', '{{ $item->name }}')"
                                                ...
                                                @endif
                                            @endcan
                                            @can('businesstypes.destroy')
                                                <button type="button"
                                                        class="bg-red-500 text-white px-2 py-1 rounded text-xs"
                                                        onclick="openModal('{{ $item->businesstype_uuid }}', '{{ $item->name }}')"
                                        ...
                                        <div class="bg-white rounded-2xl shadow-2xl w-5/6 sm:w-3/6 lg:w-2/6 xl:w-1/5 transform transition-transform scale-100 opacity-100 duration-300">
                                            <div
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="closeDetailsModal('{{$item->businesstype_uuid}}')">
                                                    &times;
                                                </button>
                                                <h1 class="text-lg font-semibold mx-auto">{{ __('word.businesstype.resource.show') }}</h1>
                                            </div>
                                            <div
                                                class="py-12 px-4 text-slate-600 rounded-b-2xl shadow-inner md:max-w-2xl">
                                                <div class="text-center">
                                                    <div>
                                                        <p class="text-sm font-semibold ">{{ __('word.businesstype.attribute.name') }}</p>
                                                        <p>{{ $item->name }}</p>
                                                    </div>
                                                    @if($item->description)
                                                        <div class="mt-2">
                                                            <p class="text-sm font-semibold ">{{ __('word.businesstype.attribute.description') }}</p>
                                                            <p>{{ $item->description }}</p>
                                                        </div>
                                                    @endif
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold ">{{ __('word.businesstype.attribute.status') }}</p>
                                                        <p>{{ $item->status ? '🟢' : '🔴' }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.businesstype.attribute.created_at') }}</p>
                                                        <p> {{ $item->created_at->format('H:i d/m/Y') }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold ">{{ __('word.businesstype.attribute.updated_at') }}</p>
                                                        <p> {{ $item->updated_at->format('H:i d/m/Y') }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold ">{{ __('word.businesstype.attribute.user_id') }}</p>
                                                        <p>{{ $item->user->name }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div id="disable-modal-{{$item->businesstype_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-disable-{{$item->businesstype_uuid}}">
                                        <div
                                            ...
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="close_disable_modal('{{$item->businesstype_uuid}}')">
                                                    &times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.businesstype.disable_confirmation')}}
                                                    <strong id="disable-name-{{$item->businesstype_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_disable_modal('{{$item->businesstype_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('businesstypes.disable',$item->businesstype_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="enable-modal-{{$item->businesstype_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-enable-{{$item->businesstype_uuid}}">
                                        <div
                                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                                        onclick="close_enable_modal('{{$item->businesstype_uuid}}')">&times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.businesstype.enable_confirmation')}}
                                                    <strong id="enable-name-{{$item->businesstype_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_enable_modal('{{$item->businesstype_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('businesstypes.enable',$item->businesstype_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="modal-{{$item->businesstype_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-delete-{{$item->businesstype_uuid}}">
                                        <div
                                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                                        onclick="closeModal('{{$item->businesstype_uuid}}')">&times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.businesstype.delete_confirmation')}}
                                                    <strong
                                                        id="name-{{$item->businesstype_uuid}}"></strong>{{__('word.general.delete_warning')}}
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="closeModal('{{$item->businesstype_uuid}}')">{{ __('Close') }}</button>
                                                <form id="delete-form-{{$item->businesstype_uuid}}"
                                                      action="{{route('businesstypes.destroy',$item->businesstype_uuid)}}"
                                                      method="POST"
                                                      onsubmit="fetch_delete(this, '{{url('/')}}', '{{ $item->businesstype_uuid }}', event)">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    <div class="pagination-wrapper mt-4">
                        {!! $businesstypes->appends(['perPage' => $perPage])->links() !!}
                    </div>
                </div>
            </div>
        </div>
