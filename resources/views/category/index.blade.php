<x-app-layout>
    ...
    <x-slot name="js_files">
        ...
        <script type="text/javascript" src="{{ asset('/js/category/fetch_delete_category.js?v='.time()) }}"></script>
    </x-slot>
    <x-slot name="header">
                    ...
                    <div class="overflow-x-auto text-black">
                        <table class="min-w-full border-collapse border-[#2563eb] text-center text-sm">
                            ...
                            <tr class="bg-[#d1d5db]" title="{{__('word.general.title_icon_filter')}}">
                                ...
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.category.attribute.user_id') }}')">{{ __('word.category.attribute.user_id') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.category.attribute.created_at') }}')">{{ __('word.category.attribute.created_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.category.attribute.updated_at') }}')">{{ __('word.category.attribute.updated_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                            ...
                            @foreach($categories as $item)
                                            ...
                                            @endcan
                                                @if($item->status)
                                                    <button type="button"
                                                            ...
                                                            onclick="open_disable_modal('{{ $item->category_uuid }}', '{{ $item->name }}')"
                                                ...
                                                @else
                                                    <button type="button"
                                                            class="bg-sky-500 text-white px-2 py-1 rounded text-xs"
                                                            onclick="open_enable_modal('{{ $item->category_uuid }}', '{{ $item->name }}')"
                                ...
                                <div id="details-modal-{{$item->category_uuid}}"
                                    ...
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-modal-{{$item->category_uuid}}">
                                        <div
                                            ...
                                            <div
                                                ...
                                                <div class="text-center">
                                                    ...
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold ">{{ __('word.category.attribute.user_id') }}</p>
                                                        <p>{{ $item->user->name }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div id="disable-modal-{{$item->category_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-disable-{{$item->category_uuid}}">
                                        <div
                                            ...
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.category.disable_confirmation')}}
                                                    <strong id="disable-name-{{$item->category_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_disable_modal('{{$item->category_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('categories.disable',$item->category_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="enable-modal-{{$item->category_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-enable-{{$item->category_uuid}}">
                                        <div
                                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                                        onclick="close_enable_modal('{{$item->category_uuid}}')">&times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.category.enable_confirmation')}}
                                                    <strong id="enable-name-{{$item->category_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_enable_modal('{{$item->category_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('categories.enable',$item->category_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="modal-{{$item->category_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-delete-{{$item->category_uuid}}">
                                        <div
                                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
