<x-app-layout>
  <x-slot name="title">
    {{ __('word.payment.meta.show.title') }}
  </x-slot>
  <x-slot name="metaDescription">
    {{ __('word.payment.meta.show.description')}}
  </x-slot>
  <x-slot name="metaKeywords">
    {{ __('word.payment.meta.show.keywords')}}
  </x-slot>
  ...
  <div class="py-12">
    <div class="mx-auto flex bg-gray-800 text-gray-200 rounded-2xl shadow-2xl overflow-hidden max-w-4xl p-8 transition-transform transform hover:scale-105 hover:shadow-lg">
      <div class="p-6 text-center">
        <h2 class="uppercase tracking-wider text-1xl text-indigo-400 font-semibold">{{ __('word.payment.resource.show') }}</h2>
        <hr class="my-4 border-gray-600">


        @if($paymentwithoutprice->observation)
        <div class="mt-6">
          <p class="text-sm font-semibold text-gray-400">{{ __('word.payment.attribute.observation') }}</p>
          <p class="text-md text-gray-200">{{ $paymentwithoutprice->observation }}</p>
        </div>
        @endif

        <div class="mt-4">
          <p class="text-sm font-semibold text-gray-400">{{ __('word.payment.attribute.created_at') }}</p>
          <p class="text-md text-gray-200"> {{ $paymentwithoutprice->created_at->format('H:i d/m/Y') }}</p>
        </div>

        <div class="mt-4">
          <p class="text-sm font-semibold text-gray-400">{{ __('word.payment.attribute.updated_at') }}</p>
          <p class="text-md text-gray-200"> {{ $paymentwithoutprice->updated_at->format('H:i d/m/Y') }}</p>
        </div>

        <div class="mt-4">
          <p class="text-sm font-semibold text-gray-400">{{ __('word.payment.attribute.user_id') }}</p>
          <p class="text-md text-gray-200">{{ $paymentwithoutprice->user->name }}</p>
        </div>
      </div>
      <div class="p-6 text-center">
