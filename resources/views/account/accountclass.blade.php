<div class="container mx-auto p-4">
    <div class="flex justify-end space-x-2 items-center mb-4">
        @can('accountclasses.create')
            <a href="javascript:void(0);"
               class="bg-blue-400 text-white px-4 py-2 rounded text-sm"
               title="{{__('word.general.title_icon_create')}}"
               onclick="open_create_modal('accountclass');">
                <i class="bi bi-plus"></i>
            </a>
        @endcan
        <form method="GET" action="{{ route('accounts.index') }}" onchange="this.submit()"
              class="inline-block">
            <select name="perpage_accountclasses" class="border border-gray-300 rounded text-sm pr-8 w-36">
                <option
                    value="10" {{ $perpage_accountclasses == 10 ? 'selected' : '' }}>{{ __('word.general.10_items') }}</option>
                <option
                    value="20" {{ $perpage_accountclasses == 20 ? 'selected' : '' }}>{{ __('word.general.20_items') }}</option>
                <option
                    value="50" {{ $perpage_accountclasses == 50 ? 'selected' : '' }}>{{ __('word.general.50_items') }}</option>
            </select>
        </form>
    </div>
    <div class="overflow-x-auto text-black">
        <table class="min-w-full border-collapse border-[#2563eb] text-center text-sm">
            ...
            <tbody>
            @foreach($accountclasses as $item)
                <tr class="hover:bg-[#d1d5db44] transition duration-200">
                    ...
                    <td class="border-t border-b border-[#d1d5db] px-2 py-1">
                        <div class="flex justify-center space-x-1">
                            @can('accountclasses.index')
                                <a href="javascript:void(0);"
                                   title="{{__('word.general.title_icon_show')}}"
                                   class="bg-green-500 text-white px-2 py-1 rounded text-xs"
                                   onclick="openDetailsModal('{{$item->accountclass_uuid}}')">
                            ...
                            @endcan
                            @can('accountclasses.edit')
                                <a href="{{ route('accountclasses.edit',$item->accountclass_uuid) }}"
                                   class="bg-yellow-500 text-white px-2 py-1 rounded text-xs"
                                   title="{{__('word.general.title_icon_update')}}"
                                   onclick="fetch_edit_accountclass(this, '{{url('/')}}', '{{ $item->accountclass_uuid }}', event);">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            ...
                            @can('accountclasses.destroy')
                                <button type="button"
                                        ...
                                        onclick="openModal('{{ $item->accountclass_uuid }}', '{{ $item->name }}')"
                        ...
                        <div
                            ...
                            <div
                                ...
                                <button type="button"
                                        ...
                                        onclick="close_edit_modal('{{$item->accountclass_uuid}}')">
                                    &times;
                                </button>
                                <h1 class="text-lg font-semibold mx-auto">{{ __('word.accountclass.resource.edit') }}</h1>
                            </div>
                            <div class="py-12 px-4 rounded-b-2xl shadow-inner md:max-w-2xl">
                                <form method="POST"
                                      action="{{route('accountclasses.update', $item->accountclass_uuid)}}">
                                    @csrf
                                    @method("PUT" )
                                    @php
                                        $accountclass->accountclass_uuid = $item->accountclass_uuid;
                                    @endphp
                                    <x-form-accountclass :accountclass="$accountclass" page="edit"/>
                ...
                <div id="details-modal-{{$item->accountclass_uuid}}"
                     class="fixed inset-0 bg-black/60 bg-opacity-50 z-50 hidden overflow-y-auto py-3">
                    <div class="flex items-center justify-center min-h-screen"
                         id="scale-modal-{{$item->accountclass_uuid}}">
                        <div
                            class="bg-white rounded-2xl shadow-2xl w-5/6 sm:w-3/6 lg:w-2/6 xl:w-1/5 transform transition-transform scale-100 opacity-100 duration-300">
                            <div
                                ...
                                <button type="button"
                                        class="close-modal text-slate-600 hover:text-gray-900 text-3xl absolute right-4"
                                        onclick="closeDetailsModal('{{$item->accountclass_uuid}}')">
                                    &times;
                                </button>
                                <h1 class="text-lg font-semibold mx-auto">{{ __('word.accountclass.resource.show') }}</h1>
                            </div>
                            <div
                ...
                <div id="disable-modal-{{$item->accountclass_uuid}}"
                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                    <div class="flex items-center justify-center min-h-screen"
                         id="scale-disable-{{$item->accountclass_uuid}}">
                        <div
                            ...
                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                ...
                                <button type="button"
                                        ...
                                        onclick="close_disable_modal('{{$item->accountclass_uuid}}')">
                                    &times;
                                </button>
                            </div>
                            <div class="modal-body p-6">
                                <p class="text-gray-600">{{__('word.accountclass.disable_confirmation')}}
                                    <strong id="disable-name-{{$item->accountclass_uuid}}"></strong>?
                                </p>
                            </div>
                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                <button type="button"
                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                        onclick="close_disable_modal('{{$item->accountclass_uuid}}')">{{ __('Close') }}</button>
                                <form
                                    action="{{route('accountclasses.disable',$item->accountclass_uuid)}}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit"
                ...
                <div id="enable-modal-{{$item->accountclass_uuid}}"
                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                    <div class="flex items-center justify-center min-h-screen"
                         id="scale-enable-{{$item->accountclass_uuid}}">
                        <div
                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                ...
                                <button type="button"
                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                        onclick="close_enable_modal('{{$item->accountclass_uuid}}')">
                                    &times;
                                </button>
                            </div>
                            <div class="modal-body p-6">
                                <p class="text-gray-600">{{__('word.accountclass.enable_confirmation')}}
                                    <strong id="enable-name-{{$item->accountclass_uuid}}"></strong>?
                                </p>
                            </div>
                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                <button type="button"
                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                        onclick="close_enable_modal('{{$item->accountclass_uuid}}')">{{ __('Close') }}</button>
                                <form
                                    action="{{route('accountclasses.enable',$item->accountclass_uuid)}}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit"
                ...
                <div id="modal-{{$item->accountclass_uuid}}"
                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                    <div class="flex items-center justify-center min-h-screen"
                         id="scale-delete-{{$item->accountclass_uuid}}">
                        <div
                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                ...
                                <button type="button"
                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                        onclick="closeModal('{{$item->accountclass_uuid}}')">&times;
                                </button>
                            </div>
                            <div class="modal-body p-6">
                                <p class="text-gray-600">{{__('word.accountclass.delete_confirmation')}}
                                    <strong
                                        id="name-{{$item->accountclass_uuid}}"></strong>{{__('word.general.delete_warning')}}
                                </p>
                            </div>
                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                <button type="button"
                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                        onclick="closeModal('{{$item->accountclass_uuid}}')">{{ __('Close') }}</button>
                                <form id="delete-form-{{$item->accountclass_uuid}}"
                                      action="{{route('accountclasses.destroy',$item->accountclass_uuid)}}"
                                      method="POST"
                                      onsubmit="fetch_delete(this, '{{url('/')}}', '{{ $item->accountclass_uuid }}', event)">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
    ...
    <div id="create-modal-accountclass"
         class="fixed inset-0 bg-black/60 bg-opacity-50 z-50 hidden overflow-y-auto py-3">
        <div class="flex items-center justify-center min-h-screen"
            ...
            <div
                ...
                <div class="modal-header p-4 bg-blue-200 text-slate-600 flex items-center justify-between rounded-t-2xl relative">
                    ...
                    <h1 class="text-lg font-semibold mx-auto">{{ __('word.accountclass.resource.create') }}</h1>
                </div>
                <div class="py-12 px-4 rounded-b-2xl shadow-inner md:max-w-2xl">
                    <form method="POST" action="{{ route('accountclasses.store') }}">
                        @csrf
                        <x-form-accountclass :accountclass="$accountclass" page="create"/>
                        <div class="mt-4 flex justify-end">
                            <x-button>
                                {{ __('Save') }}
                            </x-button>
    ...
    <div class="pagination-wrapper mt-4">
        {!! $accountclasses->appends(['perpage_accountclasses' => $perpage_accountclasses])->links() !!}
    </div>
</div>
