<div class="container mx-auto p-4">
    <div class="flex justify-end space-x-2 items-center mb-4">
        @can('mainaccounts.create')
            <a href="javascript:void(0);"
               class="bg-blue-400 text-white px-4 py-2 rounded text-sm"
               title="{{__('word.general.title_icon_create')}}"
               onclick="open_create_modal('mainaccount');">
                <i class="bi bi-plus"></i>
            </a>
        @endcan
        <form method="GET" action="{{ route('accounts.index') }}" onchange="this.submit()"
              class="inline-block">
            <select name="perpage_mainaccounts" class="border border-gray-300 rounded text-sm pr-8 w-36">
                <option
                    value="10" {{ $perpage_mainaccounts == 10 ? 'selected' : '' }}>{{ __('word.general.10_items') }}</option>
                <option
                    value="20" {{ $perpage_mainaccounts == 20 ? 'selected' : '' }}>{{ __('word.general.20_items') }}</option>
                <option
                    value="50" {{ $perpage_mainaccounts == 50 ? 'selected' : '' }}>{{ __('word.general.50_items') }}</option>
            </select>
        </form>
    </div>
    <div class="overflow-x-auto text-black">
        <table class="min-w-full border-collapse border-[#2563eb] text-center text-sm">
            ...
            @foreach($mainaccounts as $item)
                <tr class="hover:bg-[#d1d5db44] transition duration-200">
                    ...
                    <td class="border-t border-b border-[#d1d5db] px-2 py-1">
                        <div class="flex justify-center space-x-1">
                            ...
                            @endcan
                            @can('mainaccounts.edit')
                                <a href="{{ route('mainaccounts.edit',$item->mainaccount_uuid) }}"
                                   class="bg-yellow-500 text-white px-2 py-1 rounded text-xs"
                                   title="{{__('word.general.title_icon_update')}}"
                                   onclick="fetch_edit_mainaccount(this, '{{url('/')}}', '{{ $item->mainaccount_uuid }}', event);">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            ...
                            @can('mainaccounts.destroy')
                                <button type="button"
                                        ...
                                        onclick="openModal('{{ $item->mainaccount_uuid }}', '{{ $item->name }}')"
                ...
                <div id="edit-modal-{{$item->mainaccount_uuid}}"
                    ...
                    <div class="flex items-center justify-center min-h-screen"
                        ...
                        <div
                            ...
                            <div
                                ...
                                <h1 class="text-lg font-semibold mx-auto">{{ __('word.mainaccount.resource.edit') }}</h1>
                            </div>
                            <div class="py-12 px-4 rounded-b-2xl shadow-inner md:max-w-2xl">
                                <form method="POST"
                                      action="{{route('mainaccounts.update', $item->mainaccount_uuid)}}">
                                    @csrf
                                    @method("PUT" )
                                    @php
                ...
                <div id="details-modal-{{$item->mainaccount_uuid}}"
                     class="fixed inset-0 bg-black/60 bg-opacity-50 z-50 hidden overflow-y-auto py-3">
                    <div class="flex items-center justify-center min-h-screen"
                        ...
                        <div
                            ...
                            <div
                                ...
                                <h1 class="text-lg font-semibold mx-auto">{{ __('word.mainaccount.resource.show') }}</h1>
                            </div>
                ...
                <div id="disable-modal-{{$item->mainaccount_uuid}}"
                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                    <div class="flex items-center justify-center min-h-screen"
                         id="scale-disable-{{$item->mainaccount_uuid}}">
                        <div
                            ...
                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                ...
                                <button type="button"
                                        ...
                                        onclick="close_disable_modal('{{$item->mainaccount_uuid}}')">
                                    &times;
                                </button>
                            </div>
                            <div class="modal-body p-6">
                                <p class="text-gray-600">{{__('word.mainaccount.disable_confirmation')}}
                                    <strong id="disable-name-{{$item->mainaccount_uuid}}"></strong>?
                                </p>
                            </div>
                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                <button type="button"
                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                        onclick="close_disable_modal('{{$item->mainaccount_uuid}}')">{{ __('Close') }}</button>
                                <form
                                    action="{{route('mainaccounts.disable',$item->mainaccount_uuid)}}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit"
                ...
                <div id="enable-modal-{{$item->mainaccount_uuid}}"
                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                    <div class="flex items-center justify-center min-h-screen"
                        ...
                        <div
                            ...
                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                ...
                                <button type="button"
                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                        onclick="close_enable_modal('{{$item->mainaccount_uuid}}')">
                                    &times;
                                </button>
                            </div>
                            <div class="modal-body p-6">
                                <p class="text-gray-600">{{__('word.mainaccount.enable_confirmation')}}
                                    <strong id="enable-name-{{$item->mainaccount_uuid}}"></strong>?
                                </p>
                            </div>
                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                <button type="button"
                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                        onclick="close_enable_modal('{{$item->mainaccount_uuid}}')">{{ __('Close') }}</button>
                                <form
                                    action="{{route('mainaccounts.enable',$item->mainaccount_uuid)}}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit"
                    ...
                    <div class="flex items-center justify-center min-h-screen"
                        ...
                        <div
                            ...
                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                ...
                                <button type="button"
                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                        onclick="closeModal('{{$item->mainaccount_uuid}}')">&times;
                                </button>
                            </div>
                            <div class="modal-body p-6">
                            ...
                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                <button type="button"
                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                        onclick="closeModal('{{$item->mainaccount_uuid}}')">{{ __('Close') }}</button>
                                <form id="delete-form-{{$item->mainaccount_uuid}}"
                                      action="{{route('mainaccounts.destroy',$item->mainaccount_uuid)}}"
                                      method="POST"
                                      onsubmit="fetch_delete(this, '{{url('/')}}', '{{ $item->mainaccount_uuid }}', event)">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
        ...
        <div class="flex items-center justify-center min-h-screen" id="scale-modal-mainaccount">
            <div
                ...
                <div
                    ...
                    <h1 class="text-lg font-semibold mx-auto">{{ __('word.mainaccount.resource.create') }}</h1>
                </div>
                <div class="py-12 px-4 rounded-b-2xl shadow-inner md:max-w-2xl">
    ...
    <div class="pagination-wrapper mt-4">
        {!! $mainaccounts->appends(['perpage_mainaccounts' => $perpage_mainaccounts])->links() !!}
    </div>
</div>
