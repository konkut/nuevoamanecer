<x-app-layout>
    ...
    <x-slot name="js_files">
        ...
        <script type="text/javascript" src="{{ asset('/js/service/fetch_delete_service.js?v='.time()) }}"></script>
    </x-slot>
    <x-slot name="header">
            ...
            <div class="overflow-hidden shadow-xl sm:rounded-lg">
                <div class="container mx-auto p-4">
                    <div class="flex justify-end space-x-2 items-center mb-4">
                        ...
                        @endcan
                        <form method="GET" action="{{ route('services.index') }}" onchange="this.submit()"
                              class="inline-block">
                        ...
                        <table class="min-w-full border-collapse border-[#2563eb] text-center text-sm">
                            ...
                            <tr class="bg-[#d1d5db]" title="{{__('word.general.title_icon_filter')}}">
                                ...
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.service.filter.name') }}')">{{ __('word.service.attribute.name') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.service.filter.amount') }}')">{{ __('word.service.attribute.amount') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.service.filter.commission') }}')">{{ __('word.service.attribute.commission') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.service.filter.category_uuid') }}')">{{ __('word.service.attribute.category_uuid') }}</th>
                                @if(auth()->user()->hasRole('Administrador'))
                                    <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                        onclick="enableSearch(this, '{{ __('word.service.filter.user_id') }}')">{{ __('word.service.attribute.user_id') }}</th>
                                @endif
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.service.filter.created_at') }}')">{{ __('word.service.attribute.created_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.service.filter.updated_at') }}')">{{ __('word.service.attribute.updated_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.service.filter.status') }}')">{{ __('word.service.attribute.status') }}</th>
                            ...
                            @foreach($services as $item)
                                <tr class="hover:bg-[#d1d5db44] transition duration-200">
                                    ...
                                    <td class="border-t border-b border-[#d1d5db] px-2 py-1">{{ $item->amount ? number_format($item->amount, 2) : "" }}</td>
                                    <td class="border-t border-b border-[#d1d5db] px-2 py-1">{{ $item->commission ? number_format($item->commission, 2) : "" }}</td>
                                        ...
                                        <div class="flex justify-center space-x-1">
                                            @can('services.index')
                                                <a href="javascript:void(0);"
                                                   ...
                                                   onclick="openDetailsModal('{{$item->service_uuid}}')">
                                            ...
                                            @endcan
                                            @can('services.edit')
                                                <a href="{{ route('services.edit',$item->service_uuid) }}"
                                            ...
                                            @can('services.destroy')
                                                <button type="button"
                                                        ...
                                                        onclick="openModal('{{ $item->service_uuid }}', '{{ $item->name }}')"
                                ...
                                <div id="details-modal-{{$item->service_uuid}}"
                                    ...
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-modal-{{$item->service_uuid}}">
                                        <div
                                            ...
                                            <div
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="closeDetailsModal('{{$item->service_uuid}}')">
                                                    &times;
                                                </button>
                                        ...
                                        <div
                                            ...
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="close_disable_modal('{{$item->service_uuid}}')">&times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.service.disable_confirmation')}}
                                                    <strong id="disable-name-{{$item->service_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_disable_modal('{{$item->service_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('services.disable',$item->service_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="enable-modal-{{$item->service_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-enable-{{$item->service_uuid}}">
                                        <div
                                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                                        onclick="close_enable_modal('{{$item->service_uuid}}')">&times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.service.enable_confirmation')}}
                                                    <strong id="enable-name-{{$item->service_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_enable_modal('{{$item->service_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('services.enable',$item->service_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="modal-{{$item->service_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                         id="scale-delete-{{$item->service_uuid}}">
                                        <div
                                            class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                            ...
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.service.delete_confirmation')}}
                                                    <strong
                                                        id="name-{{$item->service_uuid}}"></strong>{{__('word.general.delete_warning')}}
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="closeModal('{{$item->service_uuid}}')">{{ __('Close') }}</button>
                                                <form id="delete-form-{{$item->service_uuid}}"
                                                      action="{{route('services.destroy',$item->service_uuid)}}"
                                                      method="POST"
                                                      onsubmit="fetch_delete_service(this, '{{url('/')}}', '{{ $item->service_uuid }}', event)">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
