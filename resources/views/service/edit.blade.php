    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg w-1/3 mx-auto p-8">
        <form method="POST" action="{{route('services.update', $service->service_uuid)}}">
          ...
          <x-form-service :service="$service" :categories="$categories" />
          ...
          <div class="mt-4">
            ...
            <div class="relative">
              ...
              <select id="currency_uuid" class="pl-9 pr-3 py-2 bg-[#111827] text-white border border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring focus:ring-[#2563eb] focus:border-[#2563eb] w-full" name="currency_uuid">
                <option value="" disabled {{ old('currency_uuid', $service->currency_uuid) ? '' : 'selected' }}>
                  {{__('word.service.select_currency')}}
                </option>
                @foreach($currencies as $item)
                <option value="{{ $item->currency_uuid }}" {{ (old('currency_uuid', $service->currency_uuid) == $item->currency_uuid) ? 'selected' : '' }}>
                  {{$item->name}}
                </option>
                @endforeach
              </select>
          ...
          <div class="mt-4">
            ...
            <div class="relative flex items-center">
              <span id="toggleStatus" class="mr-2 text-gray-700 {{ $service->status ? 'text-green-500' : 'text-red-500' }}">
                {{ $service->status ? 'On' : 'Off' }}
              </span>
              <button type="button" id="toggleButton"
                class="bg-gray-300 rounded-full w-12 h-6 relative focus:outline-none transition-colors duration-200 {{ $service->status ? 'bg-green-500' : 'bg-red-500' }}"
                onclick="toggleStatus()">
                <div class="absolute top-0 left-0 w-6 h-6 rounded-full transition-transform duration-200 {{ $service->status ? 'translate-x-6 bg-green-600' : 'bg-red-600' }}"></div>
              </button>
              <input type="hidden" name="status" id="status" value="{{ $service->status ? '1' : '0' }}">
            </div>
          </div>
