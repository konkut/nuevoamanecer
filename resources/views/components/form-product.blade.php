<div>
  ...
  <div class="relative">
    ...
    <x-input id="name" class="first-element focus-and-blur pl-9 block mt-1 w-full" type="text" name="name" value="{{ old('name', $product->name?? '') }}" />
  </div>
  @error('name')
...
<div class="mt-4">
  ...
  <div class="relative">
    ...
    <x-input id="description" class="focus-and-blur pl-9 block mt-1 w-full" type="text" name="description" value="{{ old('description', $product->description?? '') }}" />
  </div>
  @error('description')
  <small class="font-bold text-red-500/80">{{ $message }}</small>
  @enderror
...
<div class="mt-4">
  ...
  <div class="relative">
    ...
    <x-input id="price" class="focus-and-blur pl-9 block mt-1 w-full" type="text" name="price" value="{{ old('price', $product->price?? '') }}" />
  </div>
  @error('price')
  <small class="font-bold text-red-500/80">{{ $message }}</small>
  @enderror
...
<div class="mt-4">
    ...
    <div class="relative">
        <i class="bi bi-cash absolute top-1.5 left-2 text-[1.3em] text-[#d1d5db]"></i>
        <x-input id="stock" class="focus-and-blur pl-9 block mt-1 w-full" type="text" name="stock" value="{{ old('stock', $product->stock?? '') }}" />
    </div>
    @error('stock')
    <small class="font-bold text-red-500/80">{{ $message }}</small>
    @enderror
