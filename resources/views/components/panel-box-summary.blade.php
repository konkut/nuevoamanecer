<div class="bg-white p-6 rounded-lg shadow-md w-52 flex-1">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            @if(count($arraycash) > 0)
                ...
                <tr>
                    ...
                    <th class="px-3 py-3 text-right text-gray-900 font-extrabold text-sm uppercase tracking-wider">{{number_format($totalcash,2) ?? "0.00"}}</th>
                </tr>
                </tfoot>
            @endif
        </table>
    ...
    <div class="text-center my-6">
        <p class="text-sm text-gray-500">{{$titlebank}}</p>
    </div>
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-100">
            ...
            @if(count($arraybank) > 0)
                ...
                <tr>
                    <th class="px-3 py-3 text-start text-gray-900 font-extrabold text-sm uppercase tracking-wider">{{$total}}</th>
                    <th class="px-3 py-3 text-right text-gray-900 font-extrabold text-sm uppercase tracking-wider">{{number_format($totalbank,2) ?? "0.00"}}</th>
                </tr>
                </tfoot>
            @endif
        </table>
