...
<div class="mt-4">
    ...
    <div class="relative">
        ...
        <select id="method_uuid"
            ...
            <option value=""
                    disabled data-name="None" {{ old('transactionmethod_uuid', $expense->method_uuid) ? '' : 'selected' }}>
                {{__('word.expense.select_method')}}
            </option>
            @foreach($methods as $item)
                <option
                    data-name="{{ $item->name }}"
                    value="{{ $item->method_uuid }}" {{ (old('method_uuid', $expense->method_uuid) == $item->method_uuid) ? 'selected' : '' }}>
                    {{$item->name}}
                </option>
            @endforeach
        </select>
