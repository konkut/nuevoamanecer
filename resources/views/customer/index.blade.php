<x-app-layout>
    <x-slot name="title">
        {{ __('word.customer.meta.index.title') }}
    </x-slot>
    <x-slot name="metaDescription">
        {{ __('word.customer.meta.index.description')}}
    </x-slot>
    <x-slot name="metaKeywords">
        {{ __('word.customer.meta.index.keywords')}}
    </x-slot>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            {{ __('word.customer.resource.index') }}
        </h2>
    </x-slot>
    @if (session('success'))
            ...
            <div class="overflow-hidden shadow-xl sm:rounded-lg">
                <div class="container mx-auto p-4">
                    <div class="flex justify-end space-x-2 items-center mb-4">
                        ...
                        @endcan
                        <form method="GET" action="{{ route('customers.index') }}" onchange="this.submit()"
                              class="inline-block">
                    ...
                    <div class="overflow-x-auto text-black">
                        <table class="min-w-full border-collapse border-[#2563eb] text-center text-sm">
                            ...
                            <tr class="bg-[#d1d5db]" title="{{__('word.general.title_icon_filter')}}">
                                ...
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.customer.attribute.created_at') }}')">{{ __('word.category.attribute.created_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.customer.attribute.updated_at') }}')">{{ __('word.category.attribute.updated_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.customer.attribute.status') }}')">{{ __('word.category.attribute.status') }}</th>
                            ...
                            @foreach($customers as $item)
                                        ...
                                        <div class="flex justify-center space-x-1">
                                            @can('customers.index')
                                                <a href="javascript:void(0);"
                                                   ...
                                                   onclick="openDetailsModal('{{$item->customer_uuid}}')">
                                            ...
                                            @endcan
                                            @can('customers.edit')
                                                <a href="{{ route('customers.edit',$item->customer_uuid) }}"
                                            ...
                                            @endcan
                                                @if($item->status)
                                                    <button type="button"
                                                            ...
                                                            onclick="open_disable_modal('{{ $item->customer_uuid }}', '{{ $item->name }}')"
                                                ...
                                                @else
                                                    <button type="button"
                                                            class="bg-sky-500 text-white px-2 py-1 rounded text-xs"
                                                            onclick="open_enable_modal('{{ $item->customer_uuid }}', '{{ $item->name }}')"
                                            ...
                                            @can('customers.destroy')
                                                <button type="button"
                                                        ...
                                                        onclick="openModal('{{ $item->customer_uuid }}', '{{ $item->name }}')"
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <button type="button"
                                                        ...
                                                        onclick="closeDetailsModal('{{$item->customer_uuid}}')">
                                                    &times;
                                                </button>
                                                <h1 class="text-lg font-semibold mx-auto">{{ __('word.customer.resource.show') }}</h1>
                                            </div>
                                            <div
                                                class="py-12 px-4 text-slate-600 rounded-b-2xl shadow-inner md:max-w-2xl">
                                                <div class="text-center">
                                                    <div>
                                                        <p class="text-sm font-semibold ">{{ __('word.customer.attribute.name') }}</p>
                                                        <p>{{ $item->name }}</p>
                                                    </div>
                                                    @if($item->email)
                                                        <div class="mt-2">
                                                    ...
                                                    @endif
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold ">{{ __('word.customer.attribute.status') }}</p>
                                                        <p>{{ $item->status ? '🟢' : '🔴' }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.customer.attribute.created_at') }}</p>
                                                        <p> {{ $item->created_at->format('H:i d/m/Y') }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold ">{{ __('word.customer.attribute.updated_at') }}</p>
                                                        <p> {{ $item->updated_at->format('H:i d/m/Y') }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold ">{{ __('word.customer.attribute.user_id') }}</p>
                                                        <p>{{ $item->user->name }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div id="disable-modal-{{$item->customer_uuid}}"
                                    ...
                                    <div class="flex items-center justify-center min-h-screen"
                                        ...
                                        <div class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            ...
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.customer.disable_confirmation')}}
                                                    <strong id="disable-name-{{$item->customer_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_disable_modal('{{$item->customer_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('customers.disable',$item->customer_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="enable-modal-{{$item->customer_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                        ...
                                        <div class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                                        onclick="close_enable_modal('{{$item->customer_uuid}}')">&times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                                <p class="text-gray-600">{{__('word.customer.enable_confirmation')}}
                                                    <strong id="enable-name-{{$item->customer_uuid}}"></strong>?
                                                </p>
                                            </div>
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="close_enable_modal('{{$item->customer_uuid}}')">{{ __('Close') }}</button>
                                                <form action="{{route('customers.enable',$item->customer_uuid)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit"
                                ...
                                <div id="modal-{{$item->customer_uuid}}"
                                     class="hidden fixed inset-0 bg-black/60 bg-opacity-50 z-50 overflow-y-auto">
                                    <div class="flex items-center justify-center min-h-screen"
                                        ...
                                        <div class="bg-white rounded-lg shadow-lg w-5/6 sm:w-3/6 lg:w-2/6 transform transition-all scale-100 opacity-100 duration-300">
                                            <div class="modal-header p-4 border-b flex justify-between items-center">
                                                ...
                                                <button type="button"
                                                        class="close-modal text-gray-500 hover:text-gray-700 text-2xl"
                                                        onclick="closeModal('{{$item->customer_uuid}}')">&times;
                                                </button>
                                            </div>
                                            <div class="modal-body p-6">
                                            ...
                                            <div class="modal-footer p-4 border-t flex justify-end space-x-2">
                                                <button type="button"
                                                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded transition duration-300 hover:bg-gray-400"
                                                        onclick="closeModal('{{$item->customer_uuid}}')">{{ __('Close') }}</button>
                                                <form id="delete-form-{{$item->customer_uuid}}"
                                                      action="{{route('customers.destroy',$item->customer_uuid)}}"
                                                      method="POST"
                                                      onsubmit="fetch_delete(this, '{{url('/')}}', '{{ $item->customer_uuid }}', event)">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
