<x-app-layout>
    <x-slot name="title">
        {{ __('word.voucher.meta.edit.title') }}
    </x-slot>
    <x-slot name="metaDescription">
        {{ __('word.voucher.meta.edit.description')}}
    </x-slot>
    <x-slot name="metaKeywords">
        {{ __('word.voucher.meta.edit.keywords')}}
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4">
            <div class="bg-white overflow-hidden shadow-xl rounded-lg w-full mx-auto p-8">
                <form method="POST" action="{{route('vouchers.update', $voucher->voucher_uuid)}}">
                    @csrf
                    @method("PUT" )
                    <x-form-voucher :voucher="$voucher" :analyticalaccounts="$analyticalaccounts" page="edit"></x-form-voucher>
                    @if ($errors->any())
