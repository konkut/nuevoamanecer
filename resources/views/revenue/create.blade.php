<x-app-layout>
    <x-slot name="title">
        {{ __('word.revenue.meta.create.title') }}
    </x-slot>
    <x-slot name="metaDescription">
        {{ __('word.revenue.meta.create.description')}}
    </x-slot>
    <x-slot name="metaKeywords">
        {{ __('word.revenue.meta.create.keywords')}}
    </x-slot>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            {{ __('word.revenue.resource.create') }}
        </h2>
    </x-slot>
    <div class="py-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl rounded-lg w-full mx-auto p-8">
                <form method="POST" action="{{ route('revenues.store') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="w-full">
                            <h1 class="text-md font-bold italic block text-center py-8">{{__('word.revenue.form_create')}}</h1>
