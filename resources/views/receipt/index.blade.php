<x-app-layout>
    <x-slot name="title">
        {{ __('word.receipt.meta.index.title') }}
    </x-slot>
    <x-slot name="metaDescription">
        {{ __('word.receipt.meta.index.description')}}
    </x-slot>
    <x-slot name="metaKeywords">
        {{ __('word.receipt.meta.index.keywords')}}
    </x-slot>
            ...
            <div class="overflow-hidden shadow-xl sm:rounded-lg">
                <div class="container mx-auto p-4">
                    <div class="flex justify-end gap-4 items-end mb-4 flex-wrap flex-col sm:flex-row">
                        <form method="GET" action="{{ route('receipts.index') }}" onchange="this.submit()"
                        ...
                        <table class="min-w-full border-collapse border-[#2563eb] text-center text-sm">
                            ...
                            <tr class="bg-[#d1d5db]" title="{{__('word.general.title_icon_filter')}}">
                                ...
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.receipt.filter.code') }}')">{{ __('word.receipt.attribute.code') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.receipt.filter.services') }}')">{{ __('word.receipt.attribute.services') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.receipt.filter.methods') }}')">{{ __('word.receipt.attribute.methods') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.receipt.filter.amount') }}')">{{ __('word.receipt.attribute.amount') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.receipt.filter.user_id') }}')">{{ __('word.receipt.attribute.user_id') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.receipt.filter.created_at') }}')">{{ __('word.receipt.attribute.created_at') }}</th>
                                <th class="border-t border-b border-[#d1d5db] px-2 py-1 cursor-pointer"
                                    onclick="enableSearch(this, '{{ __('word.receipt.filter.updated_at') }}')">{{ __('word.receipt.attribute.updated_at') }}</th>
                            ...
                            @foreach($receipts as $item)
                                @if(auth()->user()->hasRole('Administrador') || $item->user_id == Auth::id())
                                    <tr class="hover:bg-[#d1d5db44] transition duration-200">
                                        ...
                                        <td class="border-t border-b border-[#d1d5db] px-2 py-1">{{ $item->income->cashshift->user->name }}</td>
                                        ...
                                        <td class="border-t border-b border-[#d1d5db] px-2 py-1">
                                            <div class="flex justify-center space-x-1">
                                                ...
                                                <a href="javascript:void(0);"
                                                   ...
                                                   onclick="openDetailsModal('{{$item->receipt_uuid}}')">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                        ...
                                        <div class="flex items-center justify-center min-h-screen"
                                            ...
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <h1 class="text-lg font-semibold mx-auto">{{ __('word.receipt.resource.show') }}</h1>
                                                    <button type="button"
                                                            class="text-gray-600 hover:text-gray-900 text-2xl absolute top-4 right-4"
                                                            onclick="closeDetailsModal('{{$item->receipt_uuid}}')">
                                                        &times;
                                                    </button>
                                                </div>
                                                <div class="modal-body py-6 text-gray-700 overflow-hidden text-center">
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.receipt.attribute.code') }}</p>
                                                        <p>{{ $item->code }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.receipt.attribute.services') }}</p>
                                                        <p>{{ $item->services }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.receipt.attribute.methods') }}</p>
                                                        <p>{{ $item->methods }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.receipt.attribute.amount') }}</p>
                                                        <p>{{ $item->amount }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.receipt.attribute.user_id') }}</p>
                                                        <p>{{ $item->income->cashshift->user->name }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.receipt.attribute.created_at') }}</p>
                                                        <p>{{ $item->created_at->format('H:i d/m/Y') }}</p>
                                                    </div>
                                                    <div class="mt-2">
                                                        <p class="text-sm font-semibold">{{ __('word.receipt.attribute.updated_at') }}</p>
                    ...
                    <div class="pagination-wrapper mt-4">
                        {!! $receipts->appends(['perPage' => $perPage])->links() !!}
                    </div>
                </div>
            </div>
        </div>
