...
class ReceiptController extends Controller
{
    public function index(Request $request)
    {
        ...
        $receipts->each(function ($receipt) {
            ...
            $receipt->services = implode(', ', $services) ?? "";
            $receipt->methods = implode(', ', $names) ?? "";
        });
    ...
    public function store(string $income_uuid)
    {
        ...
        if ($income) {
            ...
            $income->cashregister = implode(', ', $cashregisters) ?? "";
            $income->bankregister = implode(', ', $bankregisters) ?? "";
            $income->platform = implode(', ', $platforms) ?? "";
            ...
            $income->received = $received->total;
            $income->returned = $returned->total;
            $income->data = $data;
